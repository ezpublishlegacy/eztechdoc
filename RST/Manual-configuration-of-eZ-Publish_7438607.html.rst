...
eZ Publish Platform 5.x : Manual configuration of eZ Publish
============================================================

Created by andrea.melo@ez.no, last modified by bertrand.dunogier@ez.no
on Feb 19, 2014
...
+--------------------------------------+--------------------------------------+
| USERNAME                             | The MySQL user (if no user is set    |
|                                      | up, use "root").                     |
+--------------------------------------+--------------------------------------+
| PASSWORD                             | The password that belongs to the     |
|                                      | username.                            |
+--------------------------------------+--------------------------------------+
| DATABASE                             | The name of the database, for        |
|                                      | example "my\_database".              |
+--------------------------------------+--------------------------------------+
| PATH                                 | The full path to the root directory  |
|                                      | of your eZ Publish installation, for |
|                                      | example "/opt/ezp".                  |
+--------------------------------------+--------------------------------------+
...
Cache, storage, log and configuration folders must be writable both by
the web server and the command line user. The easiest way is to give the
web server user ownership of those folders:
...
One import thing to keep in mind is that **all command line scripts must
be run as the webserver user** in order not to break permissions.

***In a development environment***, it is often easier to set the server
to run as your own user.
